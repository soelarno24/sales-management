    ...
    >
      <div className="flex items-center gap-6 flex-1">
        <div className="relative w-full max-w-md group">
          ...
          <input className="w-full bg-surface-container-low border-none rounded-xl pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary transition-all font-body outline-none" placeholder={searchPlaceholder} type="text" />
        </div>
      </div>
      <div className="flex items-center gap-4">
        ...
        <button onClick={onSettingsClick} className="w-10 h-10 flex items-center justify-center rounded-full text-on-surface-variant hover:bg-surface-container-high transition-colors duration-200 cursor-pointer">
          <Icon name="settings" />
        </button>
        <div className="h-8 w-[1px] bg-outline-variant opacity-20 mx-2"></div>
        <button onClick={onProfileClick} className="flex items-center gap-3 pl-2 hover:bg-surface-container-low rounded-lg pr-2 py-1 transition-colors cursor-pointer">
          ...
          <img className="w-9 h-9 rounded-full object-cover ring-2 ring-primary-fixed" alt="Admin avatar" src={adminAvatar || AVATAR_URL} />
        </button>
      </div>
    </header>
  );
